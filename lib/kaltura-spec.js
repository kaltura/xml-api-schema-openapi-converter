    ...
    actions.forEach(function(action) {
      ...
      parameters.forEach(function(param) {
        if (param.$.type.indexOf('Kaltura') === 0) {
          ...
          path['x-parameterGroups'].push({
            name: param.$.name,
            description: 'Object Type: `' + param.$.type + '`',
            parameters: exParams.map(p => p.name),
          });
        ...
        if (enumVals) {
          ...
          newParam.descrption = msg + (newParam.description ? '\n\n' + newParam.description : '');
...
var setInputOptions = function(swagger) {
  ...
  for (var path in swagger.paths) {
    ...
    (op.parameters || []).forEach(function(p) {
      ...
      if (p.name === 'id' && isGet) {
        addDynEnum(p, path.match(/service\/([^\/]*)\//)[1]);
        p.inputType = p.type === 'number' ? 'number' : 'text';
      }
