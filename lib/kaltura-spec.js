...
var getExpandedQueryParameters = function(param, enums, definitions) {
  ...
  var params = [];
  var addProps = function(defName, baseName, cond, ignoreSubclasses) {
    console.log('add props', defName);
    ...
    if (!ignoreSubclasses && subclasses.length) {
      ...
      console.log('add subclasses');
      subclasses.forEach(function(subName) {
        addProps(subName, baseName, {name: paramName, value: subName}, true);
      })
    }
    ...
    for (var prop in props) {
      ...
      if (propDef.$ref) {
        ...
        console.log('add ref prop');
        addProps(subDef, baseName + '[' + prop + ']', cond, true)
        return;
      }
      var newParam = {
        type: propDef.type,
    ...
    console.log('add superclasses');
    (def.allOf || []).forEach(function(base) {
      addProps(base.$ref.substring('#/definitions/'.length), baseName, cond, true);
    })
  }
  addProps(param.$.type);
  return params;
