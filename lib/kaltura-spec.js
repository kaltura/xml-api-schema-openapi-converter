...
var getExpandedQueryParameters = function(param, enums, definitions) {
  ...
  var addProps = function(props, baseName, cond) {
    ...
    for (var prop in props) {
      ...
      if (propDef.$ref) {
        ...
        return addProps(definitions[subDef].properties, baseName + '[' + prop + ']', cond)
      }
      var newParam = {
        type: propDef.type,
        in: 'query',
  ...
  if (subclasses.length) {
    ...
    var paramName = param.$.name + '[objectType]';
    params.unshift({
      name: paramName,
      in: 'query',
      enum: subclasses,
    ...
    subclasses.forEach(function(subName) {
      var sub = definitions[subName];
      addProps(sub.properties, param.$.name, {name: paramName, value: subName});
    })
  }
  return params;
}
