    ...
    actions.forEach(function(action) {
      ...
      parameters.forEach(function(param) {
        if (param.$.type.indexOf('Kaltura') === 0) {
          var exParams = getExpandedQueryParameters(param, enums, definitions);
...
var getExpandedQueryParameters = function(param, enums, definitions) {
  function getSubclasses(defName) {
    ...
    subs.forEach(function(s) {
      indirSubs = indirSubs.concat(getSubclasses(s));
    })
  ...
  var addParams = function(defName, baseName, cond, skipSuper, skipSub) {
    ...
    for (var prop in props) {
      ...
      if (oldParam) {
        oldParam['showCondition'].value = oldParam['showCondition'].value.concat(cond.value)
      } else {
    ...
    if (!skipSuper) {
      (def.allOf || []).forEach(function(base) {
        addParams(base.$ref.substring('#/definitions/'.length), baseName, cond, false, true);
      })
    }
  }
