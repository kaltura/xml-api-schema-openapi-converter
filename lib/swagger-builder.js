...
SwaggerBuilder.prototype.finish = function() {
  ...
  this.fixDefinitions();
  if (TARGET_API === 'ovp') {
    this.swagger.paths['/service/apptoken/action/startSession'].post.security = [];
  }
}
...
SwaggerBuilder.prototype.addDefinitions = function() {
  ...
  this.classes.forEach((cls) => {
    ...
    if (cls.$.abstract && cls.$.abstract !== "0") {
      let subs = this.classes.filter(sub => sub.$.base === cls.$.name).map(sub => sub.$.name);
      def.oneOf = subs.map(s => ({$ref: getRefFromClassname(s)}));
      def.discriminator = 'objectType';
      def.properties.objectType = {type: 'string', enum: subs};
    }
...
SwaggerBuilder.prototype.setInputOptions = function() {
  ...
  const adjustSchema = (schema, path) => {
    ...
    for (let prop in schema.properties || {}) {
      if (isGet) {
        if (prop === 'id') {
          addDynEnum(schema.properties[prop], path.match(/service\/([^\/]*)\//)[1]);
        } else if (prop === 'entryId') {
