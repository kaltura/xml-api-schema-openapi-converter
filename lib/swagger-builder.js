...
SwaggerBuilder.prototype.addPaths = function(services, errors) {
  ...
  services.forEach((service) => {
    ...
    actions.forEach((action) => {
      ...
      if (NO_SECURITY_NEEDED.indexOf(path.operationId) !== -1) path.security = [];
      ...
      if (result) {
        path.responses['200'].schema = getResponseSchema(result.$.type, result.$.arrayType);
      }
      if (action.throws) {
        let errResp = path.responses['x-Errors'] = {description: ""};
        action.throws.forEach(t => {
          let errName = t.$.name;
